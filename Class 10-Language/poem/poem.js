...
function cleanText(text) {
    // find any place you have [number] and get rid of it 
    // regexr.com to test out "regular expressions" lets u define sets of characters
...
async function getText(){
    return new Promise((resolve, reject) => {
        let text = [];
        osmosis.get("https://en.wikipedia.org/wiki/Copper")
            .find("p")
            //finds and returns all the paragraphs on that page
            .set("paragraph")
...
async function posPoem() {
    const paragraph = await getText();
    const posTypes = ["JJR"];
    const tokens = [];

    paragraph.forEach(pg => {
